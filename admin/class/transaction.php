   ...
   public function transactionList($data)
    {
		...
		$columner = array(
			...
			array( 'db' => 'transaction_id', 'dt' => 1,'formatter'=>function($d,$row){
                return "<b>".$d."</b><span style='cursor:pointer' class='badge badge-info' onclick=\"printer('$d')\"><i class='fa fa-print'></i></span>";
            }),
...
//            array( 'db' => 'destination_acct', 'dt' => 4),
            array( 'db' => 'dress_id', 'dt' => 4,'formatter'=>function($d,$row){
                ...
                return "<b style='font-size:16px'>".$this->getitemlabel('dress','id',$d,'name')."</b>".$is_collected;
            }),
            array( 'db' => 'dress_amount', 'dt' => 5,'formatter'=>function($d,$row){
//                return "&#8358; ".number_format($d,2);
                $rtt = ($row['is_discounted'] == "1")?"<b>".number_format($row['discount_price'])."</b><br /><strike style='color:red'>".number_format($d)."</strike>":number_format($d);
                return "&#8358; ".$rtt;
            }),
            array( 'db' => 'caution_fee',     'dt' => 6,'formatter'=>function($d,$row){
                return "&#8358; ".number_format($d,2);
            }),
            array( 'db' => 'is_returned',     'dt' => 7,'formatter'=>function($d,$row){
            ...
            array( 'db' => 'additional_request',  'dt' => 11 ),
            ...
            array( 'db' => 'transaction_id',  'dt' => 15,'formatter'=>function($d,$row)
              {
                  return ($row['is_returned'] == 0 && $row['is_collected'] == 1)?'<button class="btn btn-info" onclick="getModal(\'setup/return_dress.php?trans_id='.$d.'&op=edit&id='.$row[dress_id].'\',\'modal_div\')"  href="javascript:void(0)" data-toggle="modal" data-target="#defaultModalPrimary" >Return Dress</button>':"";
              } ),
	   ...
	   $filter_2 = ($_SESSION['role_id_sess'] == "001" || $_SESSION['role_id_sess'] == "002")?"":" AND branch_id = '$_SESSION[branch_id]'";
